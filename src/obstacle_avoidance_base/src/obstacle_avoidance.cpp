...
class ObstacleAvoidance {
	...
	protected: // ROS Callbacks
		...
		geometry_msgs::Twist findClosestAcceptableVelocity(const geometry_msgs::Twist & desired) {
			...
			double min_vd = current_velocity_.linear.x - max_linear_accel_*time_horizon_;
			double max_vd = current_velocity_.linear.x + max_linear_accel_*time_horizon_;

			double min_wd = current_velocity_.angular.z - max_angular_accel_*time_horizon_;
			...
			for (unsigned int j=0;j<n_v;j++) {
				...
				int res_d = (int) round((d + max_range_) / map_resolution_);

				for (unsigned int i=0;i<n_w;i++) {
					double w = min_w + i*angular_velocity_resolution_;
					double alpha = atan2(v,w);
					int res_alpha = ((int) round(alpha / alpha_resolution_)) % n_alpha_;

					Va(j,i) = d_alpha_(res_d, res_alpha);

					double score = 0;
					if (Va(j,i) == FREE) {
						score = exp(-(k_v_ * sqrt(v - desired.linear.x) + k_w_ * sqrt(w - desired.angular.z)));

						scores(j,i) = 128 + score*127;
